...
const PromotionPage = (props) => {
  ...
  const copyLink = () => {
    ...
    tmp_tag.value = `https://theblackdiamonds.in/#/signup/${bonus.ref_code}`;
  ...
  return (
    ...
    >
      ...
      <StyledTabs variant="fullWidth" value={tab} onChange={handleChange} aria-label="styled tabs example">
        <StyledTab label="Level 1" style={{ color: "#f90" }} />
        <StyledTab label="Level 2" style={{ color: "#f90" }} />
      </StyledTabs>
      <br />
      <Row>
        ...
        <Col md={12} className={'mt-4 ml-3 mr-3'} >
          My Promotion Link
          <span style={{ float: 'right', textDecoration: 'none', color: "#6a82fb", fontSize: "17px"}} className={'mr-5'}> https://theblackdiamonds.in/#/signup/{bonus.ref_code} </span>
        </Col>
        <Col md={12} style={{ textAlign: 'center' }} className={'mt-3'}>
